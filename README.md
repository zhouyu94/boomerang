...
##  change log
### v1.0.5
> 修改单页应用出现`http.initiator` 字段不存在或者为空的情况，具体描述如下：
>> 单页应用中，当页面第一次清除缓存加载时发现了会有一次 page load 信标和 spa_hard 信标出现，但页面正常刷新时并不会出现这样的现象。所以对这一现象做特殊处理。现象具体表现为 spa_hard 信标中没有dns开始时间或者服务器响应开始时间或者服务器响应结束时间，这些时间都记到了 page load 的信标中，此次修复是将 page load 信标和 spa_hard 信标的内容进行替换。将page load信标修改为spa_hard信标的类型，将pid变为spa_hard信标的pid（此目的是将页面中发出的请求和page load进行关联）。将 spa_hard 信标的类型变更为 spa。
> 将插件版本更新至1.0.5

### v1.0.4
> 使用 `insight-new.js` 的代码

### v1.0.3
> 重构实现代码，代码存储在 `insight-new.js`中，未进行合并
